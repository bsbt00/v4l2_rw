...
//        setup.frame_bytes = setup.frame_height * setup.frame_width*3/2;
//        setup.frame_bytes =(640 * 480) + (2 * (640/2) * (480/2));
        setup.frame_bytes = setup.frame_height * setup.frame_width*2;
...
copy_frames(struct yuv_setup setup, int dev_fd)
{
	...
	while (1) {
        ...
        fprintf(stderr, "====== fread: read_size:%d  \n", read_size);
